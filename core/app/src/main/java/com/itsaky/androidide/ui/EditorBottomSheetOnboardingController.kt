...
class EditorBottomSheetOnboardingController(
  ...
  fun attach() {
    ...
    bsBinding.pageSwitchGestureBubble.bindOnboardingTarget(drawerBubbleTarget)
    bsBinding.headerContainer.bindOnboardingTarget(headerContainerTarget)
  ...
  private fun buildStep1DrawerBubble(): OnboardingStep {
    ...
    val path = computeSwipeUpPath(bubble)
    return OnboardingStep(
      ...
      // 视觉触摸模拟: 上滑 30% 然后下滑复位 (loop=false, 播放一次)
      touchSimulator = TouchSimulator(
        type = GestureType.SWIPE,
        points = listOf(
          PathPoint(x = path.fromX, y = path.fromY, timeMs = 0),
          PathPoint(x = path.fromX, y = path.toY, timeMs = 800),       // 上滑 30%
          PathPoint(x = path.fromX, y = path.toY, timeMs = 1400),     // 停顿
          PathPoint(x = path.fromX, y = path.fromY, timeMs = 2200),    // 下滑复位
        ),
        durationMs = 2200,
        loop = false,
        showFinger = true,
  ...
  private fun buildStep2HeaderContainer(): OnboardingStep {
    return OnboardingStep(
      ...
      bubbleShape = BubbleShape.WideCard(cornerRadius = 18.dp),
      bubbleStyle = BubbleStyle.WideCard,
  ...
  private fun buildStep3SymbolInput(): OnboardingStep {
    ...
    return OnboardingStep(
      ...
      bubbleShape = BubbleShape.RoundedRectangle(cornerRadius = 20.dp),
      bubbleStyle = BubbleStyle.WideCard,
      bubblePlacement = BubblePlacement.Above,
      ...
      touchSimulator = TouchSimulator(
        ...
        points = listOf(
          PathPoint(x = path.fromX, y = path.fromY, timeMs = 0),
          PathPoint(x = path.fromX, y = path.toY, timeMs = 700),  // 上滑
          PathPoint(x = path.fromX, y = path.toY, timeMs = 1300), // 停顿
          PathPoint(x = path.fromX, y = path.fromY, timeMs = 2000), // 下滑复位
        ),
        durationMs = 2000,
        loop = false,
        showFinger = true,
