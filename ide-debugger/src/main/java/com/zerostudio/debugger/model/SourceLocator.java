...
public final class SourceLocator {
     ...
     *
     *  Phase G4: Kotlin (.kt) files: try to find and parse the corresponding
     *  .class file to get the accurate signature. Kotlin compiler stores
     *  the JVM class name in metadata. Also handles multiple top-level
     *  classes per .java file by returning all candidates.
     *
    ...
    /**
     * Phase G4: Guess ALL class signatures for a source file.
     *
     * A single .java file can contain multiple top-level classes (bad practice,
     * but legal). When installing breakpoints, we need to try ALL of them.
     *
     * For .java files: uses JavaSourceParser to extract all top-level classes.
     * For .kt files: falls back to basename (Kotlin files compile to one class
     * with the same name as the file, possibly with inner classes).
     * For other files: returns a singleton list from the existing heuristic.
     */
    @NonNull
    private List<String> guessAllClassSignatures(@NonNull String sourceFile) {
        String base = basename(sourceFile);
        if (base.endsWith(".java")) {
            ...
            if (parsed != null && !parsed.classes.isEmpty()) {
                ...
                if (!sigs.isEmpty()) return sigs;
            }
            // Fallback: single class from basename
            String name = base.substring(0, base.length() - 5);
            if (!name.isEmpty()) {
                return Collections.singletonList("L" + name + ";");
            }
            return Collections.emptyList();
        } else if (base.endsWith(".kt")) {
    ...
    /**
     * Phase G4: Check if a method is synthetic (generated by the compiler,
     * e.g., lambda bridge methods, access bridges).
     *
     * Synthetic methods should not be shown as user-code breakpoints.
     * We identify them by:
     *   - Name starting with "access$" (synthetic access methods)
     *   - Name starting with "lambda$" (lambda expression bodies)
     *   - ACC_SYNTHETIC flag (set by the compiler)
     *
     * Note: This method only checks the name. For full accuracy, the
     * access flags should be checked via ClassFileReader's method access flags.
     */
    static boolean isSyntheticMethod(@NonNull String methodName) {
        return methodName.startsWith("lambda$")
            || methodName.startsWith("access$")
            || methodName.equals("<clinit>")   // class initializer — not synthetic but not user-code
