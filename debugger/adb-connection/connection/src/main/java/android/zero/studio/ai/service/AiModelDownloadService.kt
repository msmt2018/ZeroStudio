package android.zero.studio.ai.service
...
class AiModelDownloadService : Service() {
    ...
    companion object {
        ...
        private const val ACTION_CANCEL_ALL = "android.zero.studio.ai.CANCEL_ALL_DOWNLOADS"
    ...
    private fun buildProgressNotification(progressMap: Map<String, DownloadProgress>): Notification {
        ...
        if (activeDownloads.size == 1) {
            ...
            val modelName = android.zero.studio.ai.data.local.model.ModelRegistry.findById(modelId)?.name ?: modelId
